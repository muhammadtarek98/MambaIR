...
class SwinTransformerBlock(torch.nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.drop_path = torch.nn.Dropout(p=drop_path) if drop_path > 0. else torch.nn.Identity()
...
class BasicLayer(torch.nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 drop_path=0.0, norm_layer=torch.nn.LayerNorm, downsample=None, use_checkpoint=False):
...
class SwinIR(torch.nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
                 ...
                 use_checkpoint=False, upscale=2, img_range=1.0, upsampler='', resi_connection='1conv',
                 **kwargs):
        ...
        if resi_connection == '1conv':
            self.conv_after_body = torch.nn.Conv2d(in_channels=embed_dim,out_channels= embed_dim,kernel_size= 3,stride= 1,padding= 1)
        elif resi_connection == '3conv':
            # to save parameters and memory
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            self.conv_before_upsample = torch.nn.Sequential(
                torch.nn.Conv2d(in_channels=embed_dim,out_channels= num_feat,kernel_size=3,stride= 1,padding=1),
                torch.nn.LeakyReLU(inplace=True))
            self.conv_up1 = torch.nn.Conv2d(in_channels=num_feat,out_channels= num_feat,kernel_size=3,stride=1,padding=1)
            if self.upscale == 4:
                self.conv_up2 = torch.nn.Conv2d(in_channels=num_feat, out_channels=num_feat, kernel_size=3, stride=1, padding=1)
            self.conv_hr =torch.nn.Conv2d(in_channels=num_feat, out_channels=num_feat,kernel_size= 3, stride=1, padding=1)
            self.conv_last = torch.nn.Conv2d(in_channels=num_feat, out_channels=num_out_ch,kernel_size= 3, stride=1,padding= 1)
