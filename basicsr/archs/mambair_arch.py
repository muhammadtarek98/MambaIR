...
class SS2D(torch.nn.Module):
    def __init__(
        ...
        self.in_proj = torch.nn.Linear(in_features=self.d_model, out_features=self.d_inner * 2, bias=bias,
                                       **factory_kwargs)
    ...
    def forward_core(self, x: torch.Tensor):
        ...
        L = H * W
        K = 4
        x_hwwh = torch.stack(tensors=[x.view(B, -1, L), torch.transpose(x, dim0=2, dim1=3).contiguous().view(B, -1, L)],
                             dim=1).view(B, 2, -1, L)
    ...
    resi_connection: 1conv"""
        ...
        if resi_connection == '1conv':
            ...
            self.conv_after_body = torch.nn.Sequential(
                ...
                torch.nn.Conv2d(in_channels=embed_dim // 4, out_channels=embed_dim // 4, kernel_size=1, stride=1,
                                padding=0),
                torch.nn.LeakyReLU(negative_slope=0.2, inplace=True),
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            self.conv_last = torch.nn.Conv2d(in_channels=num_feat, out_channels=num_out_ch, kernel_size=3, stride=1,
                                             padding=1)
            ...
            # for image denoising
            self.conv_last = torch.nn.Conv2d(in_channels=embed_dim, out_channels=num_out_ch, kernel_size=3, stride=1,
                                             padding=1)
...
class UpsampleOneStep(torch.nn.Sequential):
    ...
    def __init__(self, scale: int, num_feat: int, num_out_ch: int):
        ...
        m = []
        m.append(torch.nn.Conv2d(in_channels=num_feat, out_channels=(scale ** 2) * num_out_ch, kernel_size=3, stride=1,
                                 padding=1))
    ...
    def __init__(self, scale: int, num_feat: int):
        ...
        if (scale & (scale - 1)) == 0:  # scale = 2^n
            for _ in range(int(math.log(scale, 2))):
                m.append(torch.nn.Conv2d(in_channels=num_feat, out_channels=4 * num_feat, kernel_size=3, stride=1,
                                         padding=1))
                m.append(torch.nn.PixelShuffle(upscale_factor=2))
        elif scale == 3:
            m.append(
                torch.nn.Conv2d(in_channels=num_feat, out_channels=9 * num_feat, kernel_size=3, stride=1, padding=1))
