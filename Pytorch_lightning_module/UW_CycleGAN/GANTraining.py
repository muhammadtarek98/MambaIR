...
class GAN(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.current_epoch == 32:
            self.logger.experiment.add_graph(self.generator_lr,
                                             hr_image)
            self.logger.log_graph(self.generator_hr, lr_image)
            self.logger.log_graph(self.discriminator_lr,)
        fake_hr, fake_lr = self(lr_image, hr_image)
        if self.current_epoch == 32:
            self.logger.experiment.add_graph(self.generator_lr,
                                             hr_image)
            self.logger.log_graph(self.generator_hr, lr_image)
            self.logger.log_graph(self.discriminator_lr,fake_lr)
            self.logger.log_graph(self.discriminator_hr,fake_hr)
